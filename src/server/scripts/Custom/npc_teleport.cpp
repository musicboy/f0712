...
namespace
{
    ...
    std::string ConvertMoney(const uint32 &Money)
    {
        ...
        if (SizeStr > 4)
            Str = Str.insert(Str.length()-4, "金币");
        if (SizeStr > 2)
            Str = Str.insert(Str.length()-2, "银币");
        Str += "铜币";
    ...
    void ActionTeleport(Player * const player, Creature * const creature, const uint32 &id)
    {
        ...
        {
            std::string msg ("你等等级不够，你需要达到等级为 " + ConvertStr(dest.m_level) + ".");
        ...
        {
            std::string msg ("你没有足够的钱，需要的金钱为 " + ConvertMoney(dest.m_cost) + ".");
            creature->MonsterWhisper(msg.c_str(), player->GetGUID());
            return;
        }
...
bool OnGossipHello(Player *player, Creature *creature)
{
    ...
    if(player->isInCombat())
    {
        ...
        creature->MonsterWhisper("你在战斗中 无法使用", player->GetGUID());
        return true;
    }
